...
class Calculator extends Component {
  ...
  onClickDot = () => {
    ...
    if (waitingForOperand){
      ...
      this.setState({
        expression: expression.indexOf('.') === -1 ? expression + '.' : expression
      })
  }
  ...
  onClickOperator = (opr) => {
    ...
    if (savedValue == null) {
      ...
      const computedVal = operations[operator](savedValue, parseFloat(expression))
      this.setState({
    ...
    return (
      <div>
          ...
          <div className='row'>
            ...
            <button className = 'number' onClick = {() => this.onClickDigit(2)}>2</button>
          ...
          <div className='row'>
            ...
            <button className = 'number' onClick = {() => this.onClickDigit(6)}>6</button>
          ...
          <div className='row'>
            <button className = 'operator' onClick = {() => this.cleanExp()}>c</button>
            <button className = 'number' onClick = {() => this.onClickDigit(0)}>0</button>
            <button className = 'number' onClick = {() => this.onClickDigit(9)}>9</button>
            <button className = 'dot' onClick = {() => this.onClickDot()}>.</button>
            <button className = 'operator' onClick = {() => this.onClickOperator('*')}>&times;</button>
            <button className = 'operator' onClick = {() => this.onClickOperator('/')}>&divide;</button>
            <button className = 'result' onClick = {() => this.onClickOperator('=')}>=</button>
          </div>
      </div>
    );
  }
